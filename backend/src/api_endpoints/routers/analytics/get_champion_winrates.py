...
from src.resources.riot_cache_keys import winrates_key, TTL_WINRATES
...
async def get_champion_winrates(
    ...
    async def _fetch() -> Dict[str, Any]:
        # Prefer shared Data Dragon cache when Redis is enabled
        from src.resources.riot_cache_keys import ddragon_champions_key, TTL_DDRAGON
        ...
        raw = await cached_or_fetch(
            ddragon_champions_key(CURRENT_PATCH),
            TTL_DDRAGON,
            _fetch_champions_catalog,
            log_prefix="get_champion_winrates",
        )
        ...
        for champ_id, champ_info in champions_data.items():
            ...
            if "Assassin" in tags:
                base_win_rate += random.uniform(-3, 5)
            elif "Tank" in tags:
                base_win_rate += random.uniform(-1, 3)
            elif "Marksman" in tags:
                base_win_rate += random.uniform(-2, 4)
            elif "Mage" in tags:
                base_win_rate += random.uniform(-2, 6)
            elif "Support" in tags:
                base_win_rate += random.uniform(0, 3)
            elif "Fighter" in tags:
                base_win_rate += random.uniform(-1, 4)
            ...
            if role_match:
                champion_stats.append({
                    ...
                    "games_played": int(pick_rate * 1000),
        ...
        log_handler.info(
            f"[get_champion_winrates] Generated champion win rates for {total_champions} champions (rank: {rank}, role: {role})"
        )
        return result

    try:
        return await cached_or_fetch(
            winrates_key(rank, role, sort_by, limit),
            TTL_WINRATES,
            _fetch,
            log_prefix="get_champion_winrates",
        )
